...
详见 [`wc-match-broadcast`](../wc-match-broadcast/SKILL.md)「并入主流程」（子项目位于 `wc_runs/data_processed/match_broadcast/`）：没做过→自动完整跑；做过→新信息**扩充 raws + LLM 判断**有实质增量才改 ours；该场 raws **归档进 `data_raw/<收集时分>/match_broadcast/<场次>/`**（块A 吃所有信息，主项目 raw 必须完整；`archive_broadcast_raws()` 幂等只增）。开关 `config/wc_pipeline.json` 默认 off。
