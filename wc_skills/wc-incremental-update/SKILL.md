...
- **版本链**：`base`(赛前) →复制→ `2026-06-11_R1` →复制→ `2026-06-15_R2` … 每个新快照 = **复制上一个最新版 + integrate 本轮新踢的比赛**；起点 `base` 永久不动。
...
- 只增量 **已踢完、非待预测** 的场次；
- 按预测点出快照 `team_data/<日期>_R<轮>/`（每个快照 = 该轮开赛前的干净信息）；
...
每轮三步：
1. **复制最新版** → 新快照：`python3 snapshot_round.py --name 2026-06-15_R2`（自动找 team_data 最新日期版、cp 成新快照 + `_snapshot.md`）
2. 对本轮**已踢完**的每场，跑下面「三步流程」，`integrate_match --td 2026-06-15_R2` 加进**这个新快照**
...
python3 snapshot_round.py --name 2026-06-15_R2
...
# ③ 增量整合进 summary
python3 integrate_match.py /tmp/m.json --ts 2026-06-11_2300 --today 2026-06-11 --td 2026-06-11_R1
```
