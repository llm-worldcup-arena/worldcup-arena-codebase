...
- **赛前 / 赛中文字动态（伤病 / 停赛 / 退赛 / 名单）**：
  ...
  - 🌍 **每轮收集范围 = 全部 48 队 · 统一标准(不分深/轻)**：范围与每队最小源数的**唯一权威 = [`bg_collect/news_preflight.py`](../../wc_eval/bg_collect/news_preflight.py)**(`MIN_SOURCES=3`)。skill 不另立标准、以它为准。
    ...
    - 🚧 **硬门禁(代码强制,不靠自觉)**:`news_preflight.py` 查 48/48 队、每队仓库逐字源 ≥3,不达标 `exit 1`;**matchday ⑦.0a 预测前强制跑它,过不了门就不预测**;`news_pipeline --teams ALL` 开头 `assert len==48`;`collect_team_news` 每队收完打印「仓库累计 X 源 ✓/✗」。**downscale 在代码层做不到。**
