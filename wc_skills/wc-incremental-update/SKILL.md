...
- 🚨 **快照命名铁规 = `YYYY-MM-DD_HHMM`(日期_时分,与预测批次同规范)** 或基底 `YYYY-MM-DD_base`；`snapshot_round.py` **强制校验**,`_R1`/`_rerun` 之类花名直接报错。"本轮第几/含哪些已踢"写进 `_snapshot.md` 说明,不进目录名。
  - ⏰ **时区铁规(2026-06-15 踩坑)**:`HHMM` 一律用**服务器/本地时间**(`date +%Y-%m-%d_%H%M`,即 matchday `--ts` 默认 `datetime.now()`),**全程一致**。**绝不混用美东(ET)**——曾用 `datetime.now(America/New_York)` 给某轮命名,结果 ET 09:18 排在服务器 16:24 之前,"最新的"反而不在最后、`snapshot_round` 复制错版本。**ET 只用于比赛 kickoff_ts(那是真实开球时间),不用于轮次/快照命名。**
...
- **赛前 / 赛中文字动态（伤病 / 停赛 / 退赛 / 名单）**：
  ...
  - 🌍 **每轮收集范围 = 全部 48 队 · 统一标准(不分深/轻)**：范围与每队最小源数的**唯一权威 = [`bg_collect/news_preflight.py`](../../wc_eval/bg_collect/news_preflight.py)**(`MIN_SOURCES=3`)。skill 不另立标准、以它为准。
    - ⚠️ **历史教训(2026-06-15 定)**:本文档曾写"比赛队深抓 / 非比赛队轻扫",而 py(collect_team_news / news_pipeline)对所有队**一视同仁** → "按 py" 成了歧义指令、每轮都被降级。**现统一:py 是权威,48 队同一标准,不分深浅。**(比赛队的额外动作=赛事播报 match_broadcast + 比赛环境,见下;但 news 标准对 48 队一致。)
    - 🚧 **硬门禁(代码强制,不靠自觉)**:`news_preflight.py` 三维度查——① **范围+数量**:48/48 队、每队仓库逐字源 ≥3;② **本轮尝试制**(`--round <ts>`):每队本轮都有"搜过"留痕 `data_raw/<ts>/_news_attempts/<队>.json`(urls_checked≥1)——**这才消"假绿灯"**:设计"全搜只记新"下 0 新增合法,但"搜了没新"必须留痕、"没搜"=无留痕=`exit 1`;③ **陈旧告警**:某队最新源 >5 天 → 黄灯(库存够但情报旧,防拿陈年伤停名单预测)。**matchday ⑦.0a 预测前强制 `news_preflight.py --round <ts>`,过不了门就不预测**;`news_pipeline --teams ALL` 开头 `assert len==48`;`collect_team_news` 每队收完打印「仓库累计 X 源 ✓/✗」+ 写本轮搜过留痕。**downscale / 假绿灯 在代码层都做不到。**
    - 某队确无足量新闻(小国/刚淘汰)→ `news_preflight.py --allow-below <队码>` 显式放行 + CHANGELOG 记明,**不许默默少收**。无新增的队照样过一遍(仓库挡重复 URL),CHANGELOG 记一笔。
