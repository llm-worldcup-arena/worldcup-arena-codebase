...
- **淘汰赛列表按开球时间排序(2026-06-29 固化)**:今日赛程/竞猜卡/赛程页/擂台 4 处淘汰赛渲染按【开球时间】排,**不按场次编号**——FIFA match 编号按括号位置定、不按开球时间(如巴西-日本=M76 当天最早 13:00 却编号最大;**编号是 FIFA 官方的,绝不能改**,改了预测/结算/晋级链全乱)。有 `update_web.py` UI 契约检查守护(擂台 koRows 须 `sort by m[3]` + 今日/竞猜卡/赛程页须有 `T(a[3])-T(b[3])` 排序≥3 处),被回归则推送前硬拦。
